...
void DumpList (List *lst, FILE *listDump) {
    ...
    for (int i = 0; i < lst->size; i++) {
        fprintf (listDump, "   %d [fillcolor=red, style=\"rounded,filled\" label=\"data: %s|{next: %d|prev: %d}\"];\n\n",
    ...
    for (int i = lst->size; i < lst->capacity; i++) {
        fprintf (listDump, "   %d [fillcolor=green, style=\"rounded,filled\" label=\"data: %s|{next: %d|prev: %d}\"];\n\n",
