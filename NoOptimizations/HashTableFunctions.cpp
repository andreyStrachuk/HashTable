...
int HashTableInit (HashTable *table, const int size) {
    ...
    table->listArr = (List *)calloc (size, sizeof (List));
    ASSERT_OK (table->listArr == nullptr, return ALLOC_FAILED;)

    for (int i = 0; i < size; i++) {
        InitList (&table->listArr[i]);
    }
...
int FillHashTable (HashTable *table, const char *fileName, int (* HashFunction)(void *, int)) {
    ...
    char *text = GetBuffer (fileName);
    int buffLen = strlen (text);
    ModifyBuffer (text);

    FillBuffer (table, text, buffLen, HashFunction);
...
int FillBuffer (HashTable *table, char *buffer, const int length, int (* HashFunction)(void *, int)) {
    ...
    do {
        buffer = FindFirstLetter (buffer);
        int lengthSub = strlen (buffer);

        HashTableInsert (table, buffer, lengthSub, HashFunction);

        buffer += lengthSub;
        buffer++;
        i++;
    }
    while (*buffer != '\0' && i < length);
...
int HashTableInsert (HashTable *table, char *str, int length, int (* HashFunction)(void *, int)) {
    ...
    unsigned int hash = HashFunction(str, length);
...
int ModifyBuffer (char *str) {
    ...
    while (*str != '\0') {
        str = FindFirstLetter (str);
        str = FindLastLetter (str);
...
char *GetBuffer (const char *fileName) { 
    ...
    ASSERT_OK (base == nullptr, return nullptr; )

    int fileSize = GetFileSize (base);

    char *text = (char *)calloc (fileSize + 1, sizeof (char));
    ASSERT_OK (text == nullptr, return nullptr; )

    fileSize = fread (text, sizeof (char), fileSize, base);
...
int HashLength (void *str, const int length) {
    assert (str);

    char *str1 = (char *)str;

    return *str1 / *str1 * length % HASHTABLESIZE; 
}
